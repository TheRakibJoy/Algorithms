...
/** Here, in map we can use hash value of string instead of string directly **/
...
priority_queue<int>cost[100005];

map<int,int>mxfre;

int main()
{
    int i,j,k,n,len,val,cnt=0,id1,id2,mx=0;
    ll ans = 0 , val1 , val2 , mn = 0;
    scin2(n , len);
    for(i=1 ; i<=n ; i++){
        string temp,temp2;
        cin>>temp>>val;
    ...
    while(i <= cnt){
        ...
        if(cost[id1].empty() || cost[id2].empty()){
            i++;
            continue;
        }
        if(id1==id2 && (int)cost[id1].size()<2){
            i++;
            continue;
        }
        val1 = cost[id1].top();
    ...
    for(i=1 ; i<=cnt ; i++){
        ...
        string rev1 = pal[s1];
        if(s1==rev1 && !cost[i].empty() && cost[i].top()>0){
            val1 = cost[i].top();
            mx = max((ll)mx , val1);
        }
    }
