...
pii edges[sz],hashvalue[sz];

pii getUnorderedHash(int node)
{
    int xx1=prime1,xx2=prime2;
    for(int j=0 ; j<(int)graph[node].size() ; j++){
        int temp = (graph[node][j]+prime1)%MOD1;
        xx1 = (xx1*temp)%MOD1;

        temp = (graph[node][j]+prime2)%MOD2;
        xx2 = (xx2*temp)%MOD2;
    }
    return make_pair(xx1,xx2);
}
...
int main()
{
    ...
    for(i=1 ; i<=m ; i++){
        ...
        if(!vis[u]){
            ...
            hashvalue[u] = getUnorderedHash(u);
        }
        if(!vis[v]){
            vis[v] = 1;
            graph[v].pb(v);
            sort(graph[v].begin() , graph[v].end());
            hashvalue[v] = getUnorderedHash(v);
        }
        if(hashvalue[u] == hashvalue[v])
            ans++;
    }
    cout<<ans<<endl;
    return 0;
