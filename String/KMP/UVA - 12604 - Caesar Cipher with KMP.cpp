...
void Solve(int t)
{
    ...
    string alpha,word,encry,s="",decry;
    cin>>alpha>>word>>encry;
    len1 = (int)alpha.size();
    len2 = (int)word.size();
    len3 = (int)encry.size();
    vi ans;
    for(i=0 ; i<len1 ; i++){    ///decrypt the string by shifting i
        map<char , char>m;
        for(j=0 ; j<len1 ; j++){
            int newid = (j - i + len1)%len1;
            m[alpha[j]] = alpha[newid];
        }
        decry = "";
        for(k=0 ; k<len3 ; k++){
            decry += m[encry[k]];
