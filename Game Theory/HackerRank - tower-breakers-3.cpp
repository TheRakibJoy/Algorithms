...
/** IF we break a bamboo into several parts, then grundy value of that bamboo will equal to XOR of grundy value of all the broken parts **/

int grundy[100005];

int generateGrundy(int n)
{
    if(grundy[n] != -1)
        return grundy[n];
    set<int>st;
    for(int i=1 ; i*i<=n ; i++){
        if(n%i == 0){
            ...
            int numoftow = y;
            if(numoftow > 1){
                int ret = generateGrundy(newheight);
                if(numoftow%2)
                    st.insert(ret);     /** XOR of odd number of x = x **/
                else
                    st.insert(0);       /** XOR of even number of x = 0 **/
            }
