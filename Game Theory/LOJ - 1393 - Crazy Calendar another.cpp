...
void Solve(int t)
{
    ...
    for(i=1 ; i<=row ; i++){
        for(j=1 ; j<=col ; j++){
            scln(mat[i][j]);
            ll dis = (row-i)+(col-j);
            if(dis%2 == 0)  ///IF this cell's distance from lower right cell is even then this cell haven't any contribution to result. Because IF second player give a move from this cell i just take those coin 1 unit closer to the target cell & finally it will reach target cell by the moves of mine. So, it doesn't matter. **/
                ;
            else
                XOR ^= mat[i][j];
        }
