...
/** Number of trailing zero in n! where base = b **/
vector<ll>Prime;
bool mark[10000009];
void sieve(ll n)
{
...
ll frequency(ll n,ll factor)    /** Frequency of a factor in n! **/
{
    ll cnt=0;
    while(n)
    {
...
int main()
{
    ...
    sieve(1000099);
    ll i,j,k,n,b;
    ull ans=9999999999999999999;
    scln2(n,b);
    Primefactorize(b);
    for(i=0;i<List.size();i++)
    {
        fren[List[i]] += frequency(n,List[i]);
        fren[List[i]] /= freb[List[i]];
        ans = min(ans,(ull)fren[List[i]]);
    }
    pf("%llu\n",ans);
    return 0;
}
