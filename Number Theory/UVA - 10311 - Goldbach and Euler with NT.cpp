...
void Sieve(int n)
{
    int i,j,sqrtN=int(sqrt(n*1.0))+5;
    status[1/32] = Set(status[1/32], 1%32);
    Prime.pb(2);
    for(i=4 ; i<=n ; i+=2)
    {
...
int Bin(int ITEM)
{
    int lo=0,hi=Prime.size()-1,mid,res=0;
    while(lo <= hi)
    {
        mid=(lo+hi)/2;
        if(ITEM == Prime[mid])
...
//    Output;
    Sieve(100000000);
    int i,j,k,t,T,n,half,x,y,id,xx,yy;
    while(scin(n) != EOF)
    {
        xx=-1;
        yy=-1;
        if(n > 4)
        {
            if(n%2 == 1)
            {
                x=2;
                y=n-2;
                if(x!=y && x!=0 && y!=0 && Check(status[x/32], x%32)==0 && Check(status[y/32], y%32)==0)
                {
                    xx=x;
                    yy=y;
                }
