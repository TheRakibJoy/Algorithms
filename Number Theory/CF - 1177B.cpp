...
int main()
{
    ll i,j,k,n,len,digitlen,kth,num=1,prevnumlen=0,digitth,orgn;
    k_digit_num();
    cin>>n;
    orgn = n;
    for(i=1 ; i<=12 ; i++){
    ...
    kth = (n+digitlen-1)/digitlen;
    for(i=2 ; i<=digitlen ; i++)
        num *= 10;
    num = (num+kth-1);
    ///
    for(i=1 ; i<digitlen ; i++)
        prevnumlen += kdigitseq[i];
    prevnumlen += ((kth-1)*digitlen);
    ///
//    cout<<kth<<endl;
//    cout<<num<<endl;
//    cout<<prevnumlen<<endl;
    ...
    digitth = orgn-prevnumlen;
    cout<<digit[digitth-1]<<endl;
    return 0;
}
