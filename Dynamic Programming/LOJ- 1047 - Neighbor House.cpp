...
int dp[25][5];       /** Color, Position **/
int mat[25][5];
int r,c=3;

int fun(int pos,int state)
{
    if(pos>=0 && pos<r && state>=0 && state<c)
    {
        if(dp[pos][state] != -1)
            return dp[pos][state];
        return dp[pos][state]=min((fun(pos+1 , (state+1)%3)+mat[pos][state]),(fun(pos+1 , (state+2)%3)+mat[pos][state]));
    }
    else if(pos<0 || pos>=r)
        return 0;
    else
