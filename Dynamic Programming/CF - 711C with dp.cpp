...
ll n,m,k,c[105],p[105][105],dp[105][105][105];

ll FuN(ll pos,ll totgroup,ll lastcolor)
{
    if(pos > n){
        if(totgroup == k)
            return 0;
    ...
    if(dp[pos][totgroup][lastcolor] != -1)
        return dp[pos][totgroup][lastcolor];
    ll ret = 1e15;

    if(c[pos] == 0){
        for(ll j=1 ; j<=m ; j++){
            ret = min(ret , p[pos][j] + FuN(pos+1 , totgroup+(j != lastcolor) , j));
        }
    }
    else{
        ret = FuN(pos+1 , totgroup+(c[pos] != lastcolor) , c[pos]);
    }
    return dp[pos][totgroup][lastcolor] = ret;
}
