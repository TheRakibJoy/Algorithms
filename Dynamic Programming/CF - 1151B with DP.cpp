...
int n,m,mat[505][505],dp[505][1025];

int FuN(int row,int mask)
{
    if(row > n){
        if(mask > 0)
            return dp[row][mask] = 1;
        else
            return dp[row][mask] = 0;
    }
    if(dp[row][mask] != -1)
        return dp[row][mask];
    int ret1=0;
    for(int j=1 ; j<=m ; j++){
        ret1 |= FuN(row+1 , mask^mat[row][j]);
    }
    return dp[row][mask] = ret1;
}

void Print(int row,int mask)
{
    if(row > n){
        pf("\n");
        return;
    ...
    pf("%d ",validcol);
    Print(row+1 , mask^mat[row][validcol]);
}
