...
int ara[5005],nxt[5005];
ll dp[5005][5005];
int n,k;
ll Fun(int pos,int team)
{
    if(pos>n || team>=k)
        return 0LL;
    if(dp[pos][team] != -1)
        return dp[pos][team];
    ll ret1 = (nxt[pos]-pos+1) + Fun(nxt[pos]+1,team+1);
    ll ret2 = Fun(pos+1,team);
    return dp[pos][team] = max(ret1,ret2);
}
int main()
{
    int i,j,temp;
    ...
    for(i=1;i<=n;i++)
    {
        temp = upper_bound(ara+1,ara+n+1,ara[i]+5)-ara;
        nxt[i]=temp-1;
    }
    ans = Fun(1,0);
    cout<<ans<<endl;
