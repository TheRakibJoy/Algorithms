...
int dp[22][4],ara[22][4];   /** State-1:number of house ; State-2:Number of color **/
int main()
{
    int i,j,k,mn,t,T,n,ans;
    scin(T);
    RUN_CASE(t,T)
    {
        ms(dp,0);
        ...
        for(i=1 ; i<=n ; i++)
        {
            for(j=1 ; j<=3 ; j++){
                scin(ara[i][j]);
                dp[i][j]=ara[i][j]; /** Store the initial value for specific house & color **/
            }
        }
        for(i=2 ; i<=n ; i++)
        {
            ...
            {
                if(j == 1)
                    dp[i][j] += min(dp[i-1][2],dp[i-1][3]); /** Take minimum cost for opposite two color **/
                else if(j == 2)
