...
/**
Note: The number will be irrelevant when it contain 3 or 6 or 9 more than 17 times.Because, then it's impossible to fulfill the condition
State-1: isStart
State-2: isSmall
State-3: is there present at least one 3?
State-4: Difference between number of 3's & 6's
State-5: Difference between number of 3's & 9's
State-6: Position
**/
...
void Solve(int t)
{
    ...
    ///ans = Fun(UB)-Fun(LB-1). But, here LB is a string. So, calculate (LB-1) is complex. So, I use ans = fun(UB)-fun(LB)+(LB is a 369 number)
