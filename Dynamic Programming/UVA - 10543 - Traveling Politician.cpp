...
vi graph[55];
int n,m,k,dp[55][25]; /**State-1: Current city ; State-2: Already visited city **/

int FuN(int curr,int visited)
{
    if((curr==n-1 && visited+1>=k) || (visited+1>20)){
        if(visited+1<=20 && visited+1>=k && curr==n-1)
            return 1;
        else
            return infinity;
    }
    ...
    for(int i=0 ; i<(int)graph[curr].size() ; i++){
        int v = graph[curr][i];
        ret1 = min(ret1 , 1+FuN(v , visited+1));
    }
    return dp[curr][visited] = ret1;
}
void Clean()
{
    for(int i=0 ; i<=n ; i++)
