...
int LIS()
{
    ...
    for(i=0 ; i<n ; i++)
        L[i]=1;
    /** start from left most item and iterate right **/
    for(i=0 ; i<n ; i++)
    {
        ...
        {
            if(ara[j] > ara[i]){
                /** jth item is greater than the ith item. so,L[j]=L[i]+1 since the jth item
                can be added after the ith item. if L[j] is already greater than or equal to
                 L[i]+1 then ignore it **/
                 if(L[j] < L[i]+1)
                    L[j] = L[i]+1;
            }
        }
