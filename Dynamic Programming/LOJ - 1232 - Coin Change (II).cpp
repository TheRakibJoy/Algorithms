...
/** Hints from : https://www.youtube.com/watch?v=_fgjrs570YE&list=PLrmLmBdmIlpsHaNTPP_jHHDx_os9ItYXr&index=11   **/

/** Coin Change with Iterative DP **/
int n,amount;
int coin[105],dp[105][10005];
void CoinChange()
{
    for(int i=1 ; i<=n ; i++)   /** Take Coin[i] **/
    {
        dp[i][0]=1;
        for(int j=1 ; j<=amount ; j++)  /** Make the amount j **/
        {
            if(j >= coin[i])
                dp[i][j]=((dp[i-1][j]%MOD) + (dp[i][j-coin[i]]%MOD));
            else
                dp[i][j]=dp[i-1][j];
        }
    }
