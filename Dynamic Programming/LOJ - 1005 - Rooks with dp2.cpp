...
ll n,k,dp[35][35]; ///State-1: row I'm staying now ; State-2: rook still I've not put

ll FuN(ll rowpos,ll kbaki)
{
    if(rowpos > n || kbaki <= 0){
        if(kbaki == 0)
            return 1LL;
    ...
    if(dp[rowpos][kbaki] != -1)
        return dp[rowpos][kbaki];
    ll ret1 = 0 , ret2 = 0 , colbaki = n-(k-kbaki);

    ret1 = colbaki * FuN(rowpos+1 , kbaki-1);

    if(n-rowpos+1 > kbaki)
        ret2 = FuN(rowpos+1 , kbaki);
    return dp[rowpos][kbaki] = ret1+ret2;
}
