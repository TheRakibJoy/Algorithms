...
int dp[188][15][1121];
int fun(int i,int k,int amount)
{
    if(i>=188 || k==0)
    {
        if(amount==0 && k==0)
    ...
    if(dp[i][k][amount] != -1)
        return dp[i][k][amount];
    int ret1=0,ret2=0;
    if(amount-Prime[i]>=0 && k>0)
        ret1=fun(i+1,k-1,amount-Prime[i]);
    ret2=fun(i+1,k,amount);
    return dp[i][k][amount]=ret1+ret2;
}
