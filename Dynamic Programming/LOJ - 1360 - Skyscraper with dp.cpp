...
int BS(int ITEM)
{
    int lo=1,hi=n,res=n+1,mid;
    while(lo <= hi)
    {
        mid = (lo+hi)/2;
        if(ITEM == ara[mid].start)
        {
            lo = mid+1;
        }
        else if(ITEM < ara[mid].start)
        {
            res = mid;
            hi = mid-1;
        }
...
int FuN(int pos)
{
    ...
    ret1 = ara[pos].profit + FuN(BS(ara[pos].start+ara[pos].height-1));
    ret2 = FuN(pos+1);
...
int main()
{
    ...
    RUN_CASE(t,T)
    {
        ...
        scin(n);
        rep(i,1,n)
        sc("%d %d %d",&ara[i].start, &ara[i].height, &ara[i].profit);
