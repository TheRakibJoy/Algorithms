...
/** At least how many coins needed to make AMOUNT **/
/** Recursive DP **/
int coin[]={1,3,5,6,9,11};
int make,n=6;
int dp[105][1005];
int ChangeMake(int i,int amount)
{
    if(i >= n)
    {
        if(amount == make)
    ...
    if(amount+coin[i] <= make)
        ret1=1+ChangeMake(i,amount+coin[i]); /** Take the coin **/
    ret2=ChangeMake(i+1,amount);             /** Ignore the coin **/
