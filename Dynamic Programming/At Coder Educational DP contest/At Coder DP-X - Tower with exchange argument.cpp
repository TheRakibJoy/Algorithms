...
{
    return (p1.wt + p1.st) < (p2.wt + p2.st);
}

vector<info>v;
ll n,dp[1005][10005];

ll FuN(ll pos,ll wtsofar)
{
    if(pos >= n)
        return 0LL;
    if(dp[pos][wtsofar] != -1)
        return dp[pos][wtsofar];
    ll ret1=0 , ret2=0;
    if(wtsofar <= v[pos].st){
        if(wtsofar+v[pos].wt <= 1e4)
            ret1 = v[pos].val + FuN(pos+1 , wtsofar+v[pos].wt);
        else
            ret1 = v[pos].val + FuN(n , wtsofar+v[pos].wt);
    }
    ret2 = FuN(pos+1 , wtsofar);
    return dp[pos][wtsofar] = max(ret1 , ret2);
}

void Solve(int t)
{
    ll i,j,k,ans,w,s,value;
    cin>>n;
    for(i=0 ; i<n ; i++){
        cin>>w>>s>>value;
        v.pb(info(w , s , value));
