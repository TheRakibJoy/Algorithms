...
/** Problem:
    ...
    Find the maximum possible length of wavio sequence
**/
#define sz 100005
int n,L[2][sz],I[sz];
vi ara;
/**  Here, L[i] means from 0 to i we can build L[i] length LIS   **/
void LISNlogK(int state)
{
    int i;
    I[0] = -infinity;
    for(i=1 ; i<=n ; i++){
    ...
    for(i=0 ; i<n ; i++){
        ...
        I[lo] = ara[i];
        L[state][i] = lo;   /** Store LIS from starting to ith position **/
        if(LisLen < lo)
            LisLen = lo;    /** Store overall LIS **/
    }
}
void Solve(int t)
{
    ...
    LISNlogK(1);    /** Reverse LIS means LDS -_- **/
    for(i=0,j=n-1 ; i<n ; i++,j--){ /** Here,L[0][i] means LIS from 0 to i ; L[0][j] means LIS from n-1 to j
            or LDS from j to n-1 **/
    /** From every position we've to take LIS from starting & LDS till ending of maximum possible same length **/
        int mn = min(L[0][i] , L[1][j]);
        ans = max(ans , (2*mn)-1);
    }
    pf("Case %d: %d\n",t,ans);
}
