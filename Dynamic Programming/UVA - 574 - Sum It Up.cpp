...
int FuN(int pos,int sum)
{
    ...
    if(dp[pos][sum] != -1)
        return dp[pos][sum];
    int ret1=0,ret2=0;
    ret1 = FuN(pos+1 , sum+ara[pos]);
    ret2 = FuN(pos+1 , sum);
    return dp[pos][sum] = ret1|ret2;
}
void PrintAll(int pos,int sum)
{
    if(pos > n)
    {
        if(sum == val)
    ...
    int ret1=dp[pos+1][sum+ara[pos]] , ret2=dp[pos+1][sum];
    if(ret1)
    {
        temp.pb(ara[pos]);
        PrintAll(pos+1 , sum+ara[pos]);
        temp.pop_back();
    }
    if(ret2)
    {
