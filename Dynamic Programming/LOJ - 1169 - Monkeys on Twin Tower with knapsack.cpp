...
int n,cost[2][1002],Time[2][1002],dp[2][1002];
int FuN(int presState,int pos)
{
    if(pos > n)
        return 0;
    if(dp[presState][pos] != -1)
        return dp[presState][pos];
    int ret1=infinity,ret2=infinity;
    ret1 = cost[presState][pos] + FuN(presState , pos+1);
    ret2 = cost[presState][pos]+Time[presState][pos] + FuN(presState^1 , pos+1);
    return dp[presState][pos] = min(ret1 , ret2);
}
void Solve(int t)
{
    int i,ans = infinity;
