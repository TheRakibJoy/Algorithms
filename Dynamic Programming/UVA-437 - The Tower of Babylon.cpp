...
struct Data
{
    ...
    {
        if(base1 != A.base1)
            return base1>A.base1;
        else if(base2 != A.base2)
            return base2>A.base2;
        return 0;
    }
}ara[10005];
int sz;
...
int LIS(int u)
{
    ...
    {
        if(ara[v].base1<ara[u].base1 && ara[v].base2<ara[u].base2)
        {
            if(LIS(v) > maxi)
                maxi=LIS(v);
        }
    }
    return dp[u]=ara[u].height+maxi;
}
int main()
{
//    Output;
    int i,j,n,x,y,z,ans,test=0,key;
    while(1)
    {
        scin(n);
        if(n == 0)
        ...
        sort(ara+1,ara+key+1);
        sz=key;
        for(i=1;i<=key;i++)
        {
//            cout<<ara[i].base1<<" "<<ara[i].base2<<" "<<ara[i].height<<endl;
            if(LIS(i) > ans)
                ans=LIS(i);
        }
