...
ll FuN(ll islastreversed,ll isvalid,ll pos){
//    cout<<isvalid<<endl;
    if(pos>n){
        if(isvalid){
            return 0;
    ...
    if(vis[islastreversed][isvalid][pos])
        return dp[islastreversed][isvalid][pos];
    ll ret1=longlimit,ret2=longlimit;

    if(islastreversed){
        if(str[pos] >= revstr[pos-1])
            ret1 = FuN(0 , isvalid , pos+1);
        else
            ret1 = FuN(0 , 0 , pos+1);
        if(revstr[pos] >= revstr[pos-1])
            ret2 = ara[pos]+FuN(1 , isvalid , pos+1);
        else
            ret2 = ara[pos]+FuN(1 , 0 , pos+1);
    }
    else{
        if(str[pos] >= str[pos-1])
            ...
            ret1 = FuN(0 , 0 , pos+1);
        if(revstr[pos] >= str[pos-1])
            ret2 = ara[pos]+FuN(1 , isvalid , pos+1);
        else
            ret2 = ara[pos]+FuN(1 , 0 , pos+1);
    }
    vis[islastreversed][isvalid][pos] = 1;
    return dp[islastreversed][isvalid][pos] = min(ret1 , ret2);
}
