...
/**
Problem: There're N wines in a row. Each year you sell either the leftmost or the rightmost
wine. The ith wine has initial price p[i]. You can sell it at k*p[i] in the kth year.
What is the maximum possible total profit? **/

int n,ara[105],dp[105][105];

/** Recursive DP **/
int FuN(int lft,int rgt)
{
    if(lft == rgt)
        return n*ara[lft];
    if(dp[lft][rgt] != -1)
        return dp[lft][rgt];
    int ret1=0,ret2=0,runningyr = (lft-1)+(n-rgt)+1;
    ret1 = (ara[lft]*runningyr)+FuN(lft+1 , rgt);
    ret2 = (ara[rgt]*runningyr)+FuN(lft , rgt-1);
    return dp[lft][rgt] = max(ret1 , ret2);
}
