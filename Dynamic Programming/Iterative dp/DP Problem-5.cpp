...
/**
Problem: Given the target value N and an array of allowed numbers, count the total number
of ways to write VALUE as the sum of those numbers. (order matters) **/

int value,n,ara[105],dp[105];

int FuN1()  ///As usual DP
{
    dp[0] = 1;
    for(int i=1 ; i<=value ; i++){
        for(int j=1 ; j<=n && i-ara[j]>=0 ; j++)
            dp[i] += dp[i-ara[j]];
    }
    return dp[value];
}
...
int FuN2()  ///Forward DP
{
    ...
    for(int i=0 ; i<=value-1 ; i++){
        for(int j=1 ; j<=n && i+ara[j]<=value ; j++)
            dp[i+ara[j]] += dp[i];
    }
    return dp[value];
}
