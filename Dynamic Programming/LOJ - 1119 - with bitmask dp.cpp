...
{
    if(mask == (1<<n)-1)
        return 0;   /** All product has been taken **/
    if(dp[mask] != -1)
        return dp[mask]; /** value for this state has already calculated **/
    int mn=1<<28,val;
    for(int i=0; i<n; i++)
    {
        if(check(mask,i) == 0)  /** Is this product has not taken before? **/
        {
            val = price[i][i];
            for(int j=0; j<n; j++)
            {
                if(i!=j && check(mask,j)!=0)
                    val += price[i][j]; /** If the product j has already taken,then increase the price of product i**/
            }
            int ret = val + Fun(Set(mask,i));   /** Product i marked as taken **/
            mn = min(mn,ret);
        }
    }
    return dp[mask]=mn;
