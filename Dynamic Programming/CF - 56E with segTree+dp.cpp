...
int tree[4*sz],prefix[sz],dp[sz];

struct info
{
    int x,h,id;
    info(){}
    info(int _x,int _h,int _id)
    {
        x = _x;
        h = _h;
        id = _id;
...
void Solve(int t)
{
    ...
    scin(n);
    for(i=1 ; i<=n ; i++){
        scin2(ara[i].x , ara[i].h);
        ara[i].id = i;
        st.insert(ara[i].x - 1);
        st.insert(ara[i].x);
        st.insert(ara[i].x + 1);
        st.insert(ara[i].x + ara[i].h - 1);
    }
    sort(ara+1 , ara+n+1);
    for(auto it : st){
        compress[it] = ++cnt;
    ...
    for(i=1 ; i<=n ; i++){
        int newval = compress[ara[i].x];
        prefix[newval] = 1;
    }
    for(i=1 ; i<=cnt ; i++)
        prefix[i] += prefix[i-1];
    for(i=n ; i>=1 ; i--){
        int lb = compress[ara[i].x + 1];
        int ub = compress[ara[i].x + ara[i].h - 1];
        int mx = max(ub , Query(1 , 1 , cnt , lb , ub));
        dp[ ara[i].id ] = prefix[mx] - prefix[compress[ara[i].x - 1]];
//        error(i , lb , ub , compress[ara[i].x] , mx);
        Update(1 , 1 , cnt , compress[ara[i].x] , mx);
    }
    for(i=1 ; i<=n ; i++)
        cout<<dp[i]<<' ';
    cout<<endl;
