...
struct info
{
    ...
    double pro;
    info(){}
    info(int _x,int _y,int _t,double _pro)
    {
        x = _x;
        y = _y;
        t = _t;
...
double FuN(int isStart,int pos,int lasttaken)
{
    if(pos > n)
        return 0.0;
    if(vis[isStart][pos][lasttaken])
        return dp[isStart][pos][lasttaken];
    double ret1=0.0 , ret2=0.0;

    if(isStart){
        ret1 = ara[pos].pro + FuN(0 , pos+1 , pos);
        ret2 = FuN(isStart , pos+1 , lasttaken);
    }
    else{
        double dis = sqrt(((ara[pos].x - ara[lasttaken].x) * (ara[pos].x - ara[lasttaken].x)) + ((ara[pos].y - ara[lasttaken].y) * (ara[pos].y - ara[lasttaken].y)));
        if(dis <= ara[pos].t - ara[lasttaken].t)
            ret1 = ara[pos].pro + FuN(isStart , pos+1 , pos);
        ret2 = FuN(isStart , pos+1 , lasttaken);
    }
    vis[isStart][pos][lasttaken] = 1;
    return dp[isStart][pos][lasttaken] = max(ret1 , ret2);
}
...
void Solve(int t)
{
    cin>>n;
    for(int i=1 ; i<=n ; i++){
        cin>>ara[i].x>>ara[i].y>>ara[i].t>>ara[i].pro;
    }
    sort(ara+1 , ara+n+1);
    double ans = FuN(1 , 1 , 0);
    pf("%0.10f\n",ans);
}
