...
{
    return (p1.atk * p2.tim) > (p2.atk * p1.tim);
}
...
{
    vector<info>vec;
    ll i,j,k,n,h,a,ans=0,sum=0;
    cin>>n;
    for(i=0 ; i<n ; i++){
        cin>>h>>a;
        sum += a;

        ll neededtim = lower_bound(reqtim.begin() , reqtim.end() , h) - reqtim.begin();
        neededtim++;

        vec.pb(info(h , a , neededtim));
    }
