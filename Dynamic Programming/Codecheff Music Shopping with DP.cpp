...
int fun(int pos,int tot)
{
    ...
    if(dp[pos][tot] != -1)
        return dp[pos][tot];
    int ret1=0,ret2=0,ret3=0,alb;
    alb=arr[pos].a;
    if(pos==ara[alb].start && ara[alb].price+tot<=P)
        ret1 = ara[alb].great + fun(pos+ara[alb].song , tot+ara[alb].price);
    if(arr[pos].p+tot<=P)
        ret2 = arr[pos].v + fun(pos+1 , tot+arr[pos].p);
    ret3 = fun(pos+1,tot);
    return dp[pos][tot] = max(ret1,max(ret2,ret3));
}
int main()
{
    int i,j,k,temp,ans;
    sc("%d %d %d",&N,&M,&P);
    for(i=1;i<=N;i++)
        sc("%d %d %d",&arr[i].a,&arr[i].p,&arr[i].v);
    sort(arr+1,arr+N+1,cmp);
    Clean();
    for(i=1;i<=N;i++)
    {
        temp=arr[i].a;
        ara[temp].start = min(ara[temp].start,i);
        ara[temp].song += 1;
        ara[temp].great += arr[i].v;
    }
    ms(dp,-1);
    for(i=1;i<=M;i++)
    {
