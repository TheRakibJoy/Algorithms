...
ll C,S,tot,E,dp[105][1005],prof[105][105];
bool canland[105],vis[105][1005];
...
ll FuN(ll pos,ll baki)
{
    ...
    if(vis[pos][baki])
        return dp[pos][baki];
    ll ret1=-1e18,temp,possible=0;
    for(ll i=1 ; i<=C ; i++){
        if(pos != i){
            temp = prof[pos][i] + FuN(i , baki-1);
            ret1 = max(temp , ret1);
            possible = 1;
        }
    }
...
int main()
{
    ...
    while(1)
    {
        sc("%lld %lld %lld %lld",&C,&S,&E,&tot);
        if(C==0 && S==0 && E==0 && tot==0)
            break;
        Solve(t);
    }
    return 0;
