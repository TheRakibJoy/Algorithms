...
ll Set(ll mask,ll pos)
{
    return mask = (mask | (1LL << pos));
}

ll n,k,ara[55],dp[55][55],prefsum[55];

ll isPossible(ll pos,ll shelfno,ll mask)
{
    if(shelfno == k){
        ll sum = prefsum[n] - prefsum[pos-1];
        if((sum & mask) == mask)
            return 1LL;
        else
            return 0LL;
    ...
    if(dp[pos][shelfno] != -1)
        return dp[pos][shelfno];
    ll ret = 0LL , temp;
    for(ll i=pos ; k-shelfno <= n-i ; i++){
        ll sum = (prefsum[i] - prefsum[pos-1]);
        ...
        temp &= isPossible(i+1 , shelfno+1 , mask);
        ret |= temp;
    }
    return dp[pos][shelfno] = ret;
}
