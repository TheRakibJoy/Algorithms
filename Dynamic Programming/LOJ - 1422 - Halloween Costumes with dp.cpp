...
/**
Problem: LOJ - 1422 - Halloween Costumes with Interval DP or MCM Category
...
int FuN(int lft,int rgt)
{
    ...
    if(dp[lft][rgt] != -1)
        return dp[lft][rgt];
    int ret = 1+FuN(lft+1 , rgt);
    for(int i=lft+1 ; i<=rgt ; i++){
        if(color[lft] == color[i])  /** IF two colors r same,then he can attend both the parties with same costumes. And I'll count it later(increment 1 on ith party) **/
            ret = min(ret , FuN(lft+1 , i) + FuN(i+1 , rgt));
    }
