...
4.  array a is sorted in non-decreasing order;
5.  array b is sorted in non-increasing order.
As the result can be very large, you should print it modulo 109+7.
1<=n<=1000 ; 1<=m<=10
Solution:
IF we make two array A and B according to the above condition then we can see that
A1,A2,...,Am,Bm,Bm-1,...,B1 is an non-decreasing array of length 2m,where each element of the array is between 1 and n
**/
ll n,m,dp[25][1005];
/// State-1: Position of sequence ; State-2: Last used value
ll FuN(ll pos,ll lastusedval)
{
    if(pos > 2*m)
        return 1;
    if(dp[pos][lastusedval] != -1)
        return dp[pos][lastusedval];
    ll ret1=0;
    for(ll i=lastusedval ; i<=n ; i++){
        ret1 = add(ret1 , FuN(pos+1 , i));
    }
    return dp[pos][lastusedval] = ret1;
}
int main()
{
    ll i,j,k,ans;
