...
ll n,m,d,ara[205];
ll dp[12][22][202];
/** State-1: Taken element so far ; State-2: Mod value of sum ; State-3: Position **/
ll FuN(ll taken,ll valaftermod,ll pos)
{
    if(pos > n){
        if(valaftermod==0 && taken==m)
            return 1LL;
        else
            return 0LL;
    }
    if(dp[taken][valaftermod][pos] != -1)
        return dp[taken][valaftermod][pos];
    ll ret1=0LL,ret2=0LL;
    if(taken < m)
        ret1 = FuN(taken+1 , (((valaftermod+ara[pos])%d)+d)%d , pos+1);
    ret2 = FuN(taken , valaftermod , pos+1);
    return dp[taken][valaftermod][pos] = ret1+ret2;
}
int main()
{
    ll i,j,k,t,T,q,ans;
