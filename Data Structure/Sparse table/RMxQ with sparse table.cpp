...
/** Range maximum Query with sparse table **/
const int sz = 100005;
const int k = 25;
int Log[sz],ara[sz],sptable[sz][k];

void PreLog(int n)  ///Pre-calculate all flooring logarithmic value
{
    Log[1] = 0;
    for(int i=2 ; i<=n ; i++)
        Log[i] = Log[i/2]+1;
...
int RmxQ(int lft,int rgt)
{
    int j = Log[rgt-lft+1];
    int ret = min(sptable[lft][j] , sptable[rgt-(1<<j)+1][j]);
    return ret;
}
