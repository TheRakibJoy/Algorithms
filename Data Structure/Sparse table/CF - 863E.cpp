...
/** Array compression + Range minimum Query with sparse table **/

#define sz 1000005
int fre[sz],l[sz],r[sz],Log[sz],sptable[sz][21];
...
{
    int j = Log[rgt-lft+1];
    return min(sptable[lft][j] , sptable[rgt-(1<<j)+1][j]);
}

void Solve(int t)
{
    set<int>st;
    int i,j,n,cnt=0,lft,rgt,ans=-1,minimum=1;
    scin(n);
    for(i=1 ; i<=n ; i++){
        scin2(l[i] , r[i]);
        st.insert(l[i]);
