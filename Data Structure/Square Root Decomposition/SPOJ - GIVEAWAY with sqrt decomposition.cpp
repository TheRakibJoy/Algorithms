...
{
    int cnt = 0 , pos = l;
    int stblock = l/blockSize;
    int edblock = r/blockSize;

    while(pos <= r && pos < (blockSize*(1+stblock))){
        if(ara[pos] >= val)
            cnt++;
        pos++;
    }

    while(pos <= r && pos+blockSize < r){
        int lb = lower_bound(block[pos/blockSize].begin() , block[pos/blockSize].end() , val) - block[pos/blockSize].begin();
        cnt += (block[pos/blockSize].size() - lb);
        pos += blockSize;
    }
...
void Update(int idx,int val)
{
    ...
    int lb = lower_bound(block[blockno].begin() , block[blockno].end() , ara[idx]) - block[blockno].begin();

    block[blockno][lb] = val;
    ara[idx] = val;
    srt(block[blockno]);
}
