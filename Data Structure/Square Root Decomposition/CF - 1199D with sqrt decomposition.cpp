...
#define sz 200009
int n,blocksz,totblock,ara[sz],blockpayout[505];

void receiptUpdate(int p,int val)
{
    int blockno = p/blocksz , lft = max(1 , blockno*blocksz) , rgt = min(n , lft+blocksz-1);
    if(blockpayout[blockno] > 0){
        for(int i=lft ; i<=rgt ; i++)
            ara[i] = max(ara[i] , blockpayout[blockno]);
        blockpayout[blockno] = 0;
    }
    ara[p] = val;
}

void payoutUpdate(int val)
{
    for(int i=0 ; i<=totblock ; i++)
        blockpayout[i] = max(blockpayout[i] , val);
}

void Solve(int t)
{
    int i,j,k,q,type,p,val;
    scin(n);
    for(i=1 ; i<=n ; i++)
        scin(ara[i]);
    ...
    for(i=0 ; i<=totblock ; i++){
        int lft = max(1 , i*blocksz);
        int rgt = min(n , lft+blocksz-1);
        if(blockpayout[i] > 0){
            for(j=lft ; j<=rgt ; j++)
                ara[j] = max(ara[j] , blockpayout[i]);
            blockpayout[i] = 0;
        }
    }
