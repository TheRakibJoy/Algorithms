...
int Bin(int ITEM)
{
    int lo=0,hi=coin.size()-1,mid,res=0;
    while(lo<=hi)
    {
        mid = (lo+hi)/2;
        if(coin[mid] == ITEM)
...
int main()
{
    ...
    create();
    int i,j,n,k,x,add=0,cnt=0;
    scin2(n,k);
    while(n>0)
    {
        x=Bin(n);
        s.insert(info(++add , coin[x]));
        n-=coin[x];
    }
    cnt = s.size();
    info temp = *s.find_by_order(0);
    while(temp.val%2==0 && cnt<k)
    {
        x = temp.val/2;
        s.erase(temp);
        s.insert(info(++add,x));
        s.insert(info(++add,x));
        temp=*s.find_by_order(0);
        cnt++;
    }
    if(cnt == k)
    {
