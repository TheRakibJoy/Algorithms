...
bool UnionSet(int nodeA,int nodeB)
{
    int a = FindSet(nodeA);
    int b = FindSet(nodeB);
    if(a != b){
        ...
        par[b] = a;
        Size[a] += Size[b];
        return 1;       /** A road is needed **/
    }
    else
        return 0;       /** Doesn't need **/
}
vector<pii>bad,add;
int main()
{
    ...
    for(i=1 ; i<=n ; i++){
        for(j=1 ; j<=n ; j++){
            ...
            v = j;
            if(UnionSet(u,v))
                add.pb(make_pair(u,v));
        }
    }
    mn = min((int)bad.size() , (int)add.size());
    cout<<mn<<endl;
    for(i=0 ; i<mn ; i++){
