...
#define sz 20009
ll n,ans,par[sz],bucksz[sz],query[sz];      ///query[i] = -1 means its Q operation; otherwise its remove operation and remove query[i]'th edge
bool isErased[sz];
vector<pll>edges;
...
void unionSet(ll u,ll v)
{
    ...
    ans -= (bucksz[a] * bucksz[b]);

    if(bucksz[a] < bucksz[b])
        swap(a , b);
    par[b] = a;
    bucksz[a] += bucksz[b];
}
...
void Solve(int t)
{
    ...
    for(i=q ; i>=1 ; i--){
        if(query[i] == -1)
            ...
            u = edges[query[i]].first;
            v = edges[query[i]].second;
            unionSet(u , v);
        }
    }
    rvs(vec);
