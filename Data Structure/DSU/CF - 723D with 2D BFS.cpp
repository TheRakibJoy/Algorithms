...
int main()
{
    ...
    for(i=0 ; i<col ; i++){
        if(grid[0][i]=='.' && vis[0][i]==0)
            BFS(0 , i);
        if(grid[row-1][i]=='.' && vis[row-1][i]==0)
            BFS(row-1 , i);
    }
    for(i=0 ; i<row ; i++){
        if(grid[i][0]=='.' && vis[i][0]==0)
            BFS(i , 0);
        if(grid[i][col-1]=='.' && vis[i][col-1]==0)
            BFS(i , col-1);
    }
    ///
    for(i=1 ; i<row-1 ; i++){
        for(j=1 ; j<col-1 ; j++){
            if(grid[i][j]=='.' && vis[i][j]==0){
                lake++;
                cnt = BFS(i,j);
                MP.push_back(info(cnt , i , j));
            }
        }
    }
    sort(MP.begin() , MP.end());
    ms(vis , 0);
//    cout<<lake<<' '<<MP.size()<<endl;
    for(i=0 ; i<(int)MP.size() ; i++){
        if(lake == k)
            break;
        ans += MP[i].fre;
        Fill_UP(MP[i].x , MP[i].y);
        lake--;
    }
    cout<<ans<<endl;
    for(i=0 ; i<row ; i++){
