...
pii LCA_Query(int n,int p,int q)
{
    ...
    ret.first = min(ret.first , min(cost[make_pair(p,parent[p])] , cost[make_pair(q,parent[q])]));
    ret.second = max(ret.second , max(cost[make_pair(p,parent[p])] , cost[make_pair(q,parent[q])]));
    return ret;
}
void Clear(int n)
{
