/** Problem Link: https://toph.co/c/inpc-fall-2018-r/arena#!/p/man-in-the-queue **/
/** Solution with order set **/
#include<bits/stdc++.h>
...
{
    orderset<node>s;
    orderset<node>::iterator it;
    ll t,T,i,j,n,q,p,v,m,cnt,w,id,x,ans;
    scln(T);
    RUN_CASE(t,T)
    {
        s.clear();
        ...
        for(i=1;i<=q;i++)
        {
            ...
            {
                scln2(p,v);
                s.erase(node(p,ara[p].w));
                ara[p].w=v;
                s.insert(node(p,ara[p].w));
            }
            else
            {
                scln(m);
