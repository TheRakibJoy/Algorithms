...
struct info
{
    double obtain,total;
    info(){}
    info(double _obtain,double _total)
    {
        obtain = _obtain;
        total = _total;
    }
...
{
    return (p1.obtain - wannaMake*p1.total) > (p2.obtain - wannaMake*p2.total);
}

bool Check(double mid,ll n,ll canUse)
{
    wannaMake = mid/100.0;
    sort(ara+1 , ara+n+1 , cmp);
    double nom=0 , denom=0;
    for(ll i=1 ; i<=canUse ; i++){
        nom += ara[i].obtain;
        denom += ara[i].total;
    }
    double avg = (nom / denom)*100.0;
    return avg >= mid;
}

void Solve(int t)
{
    ll i,j,k,n,d,nom=0,denom=0;
    scln2(n , d);
    for(i=1 ; i<=n ; i++)
        sc("%lf %lf",&ara[i].obtain,&ara[i].total);

    double lo=0.0 , hi=100.0 , mid , ans = 0.0;
