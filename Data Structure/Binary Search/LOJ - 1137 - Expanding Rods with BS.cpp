...
/** Hints: https://www.mathopenref.com/arcradiusderive.html **/
...
void Solve(int t)
{
    ...
    double lo=0.0,hi=L,mid,h,radius,centerangle,hypotenus;
    while(hi-lo > 1e-8){
        mid = (lo+hi)/2.0;
        h = mid;
        radius = (h/2.0) + ((L*L)/(8.0*h));
        centerangle = 2.0 * asin((L/2.0)/radius);
        double arc = radius * centerangle;
        if(arc >= fL)
            hi = mid;
        else
            lo = mid;
