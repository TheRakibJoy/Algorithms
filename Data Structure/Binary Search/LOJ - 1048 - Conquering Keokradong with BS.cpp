...
void result_generate(int x)
{
    ...
    {
        /** Greedy calculation **/
        int i=0,j,cnt=1,tot=0;
        for(i=1 ; i<=(n-(k+1-cnt)) ; i++)
        {
            if(tot+ara[i] > x)
            {
                ans.pb(tot);
...
int main()
{
    ...
    RUN_CASE(t,T)
    {
        ...
        ms(sum,0);
        scin2(n,k);
        int lo=1,hi=1000000000,mid,res=intlimit;
        n+=1;
        for(i=1 ; i<=n ; i++)
        {
            scin(ara[i]);
