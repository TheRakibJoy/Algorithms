...
{
    int i,j,t,T;
    double r,d,len,wid,arc,angle,a,b,tempar,par,k,lo,hi,mid;
    par=400;
    char ch;
    scin(T);
    RUN_CASE(t,T)
    {
        ...
        while(1)
        {
            ...
            len=a*mid;
            wid=b*mid;
            d=sqrt((len*len)+(wid*wid));
            r=d/2.0;
            angle = acos(((r*r)+(r*r)-(wid*wid))/(2*r*r));
            arc = (r*angle);
            tempar = (len*2.0)+(arc*2.0);
            if(abs(par-tempar) < 0.0000001)
                break;
            if(tempar > par)
                hi=mid;
            else
