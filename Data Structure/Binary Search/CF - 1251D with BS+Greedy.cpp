...
pll ara[200005];
ll n;
bool valid(ll mid , ll tot)
{
    bool fg = 1;
    ll last=0LL,used=0LL,cnt=0LL,half = (n+1)/2;
    for(ll i=n ; i>=1 ; i--){
        if(cnt < half){
            if(mid>=ara[i].first && mid<=ara[i].second){
                cnt++;
                used += mid;
            }
            else{
                used += ara[i].first;
                cnt += (ara[i].first>=mid)?1:0;
            }
        }
        else{
            used += ara[i].first;
...
int main()
{
    ...
    RUN_CASE(t,T)
    {
        ans = 0LL;
        scln2(n , s);
        rep(i , 1 , n)scln2(ara[i].first , ara[i].second);
        sort(ara+1 , ara+n+1);
