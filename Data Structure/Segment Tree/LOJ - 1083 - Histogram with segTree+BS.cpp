...
/** Maximum Rectangle Area or Histogram **/
/** Range minimum query + Binary search **/
#define sz 30005
int n,ara[sz];
pii tree[3*sz]; /** 1st element - min value in this range ; 2nd element - index of the min value **/
...
pii query(int node,int b,int e,int i,int j)
{
    ...
    int rgt=lft+1;
    int mid=(b+e)/2;
    pii p1 = query(lft,b,mid,i,j);
    pii p2 = query(rgt,mid+1,e,i,j);
    pii ret;
    ret.first = min(p1.first , p2.first);
    ret.second = (p1.first==ret.first)?p1.second:p2.second;
    return ret;
}
int Calculate(int b,int e)
{
    ...
    if(b == e)
        return ara[b];
    pii ret = query(1,1,n,b,e);
    int ret1 = ret.first*(e-b+1);
    int ret2 = Calculate(b,ret.second-1);
    int ret3 = Calculate(ret.second+1,e);
