...
/** 2D Segment tree template - 02 **/
/** Maximum element of a sub rectangle and point update **/
...
void initY(int nodeX,int bX,int eX,int nodeY,int bY,int eY)
{
    if(bY == eY)
    {
        if(bX == eX)
            ...
            tree[nodeX][nodeY] = max(tree[nodeX*2][nodeY] , tree[nodeX*2+1][nodeY]);
        return;
    }
    int midY = (bY+eY)/2;
...
int maxQueryY(int nodeX,int nodeY,int bY,int eY,int ly,int ry)
{
    if(ly > ry)
        return 0;
    if(ly==bY && ry==eY)
    ...
    int midY = (bY+eY)/2;
    int ret1 = maxQueryY(nodeX , nodeY*2 , bY , midY , ly , min(ry,midY));
    int ret2 = maxQueryY(nodeX , nodeY*2+1 , midY+1 , eY , max(ly,midY+1) , ry);
    return max(ret1 , ret2);
}
int maxQueryX(int nodeX,int bX,int eX,int lx,int rx,int ly,int ry)
{
    if(lx > rx)
        return -infinity;
    if(lx==bX && rx==eX)
        return maxQueryY(nodeX , 1 , 1 , m , ly , ry);
    int midX = (bX+eX)/2;
    int ret1 = maxQueryX(nodeX*2 , bX , midX , lx , min(rx,midX) , ly , ry);
    int ret2 = maxQueryX(nodeX*2+1 , midX+1 , eX , max(lx,midX+1) , rx , ly , ry);
    return max(ret1 , ret2);
}
void updateY(int nodeX,int bX,int eX,int nodeY,int bY,int eY,int x,int y,int val)
{
    ...
    tree[nodeX][nodeY] = max(tree[nodeX][nodeY*2] , tree[nodeX][nodeY*2+1]);   ///Merge two columns
}
...
int main()
{
    ...
    for(i=1 ; i<=q ; i++)
    {
        ...
        else        ///Max Query
        {
            scin2(x1,y1);
            scin2(x2,y2);
            ans = maxQueryX(1 , 1 , n , x1 , x2 , y1 , y2);
            cout<<ans<<endl;
        }
    }
    return 0;
