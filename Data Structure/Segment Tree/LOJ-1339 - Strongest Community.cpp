...
#define sz 100005
int fre[sz],arr[sz],lft[sz],rgt[sz],val[sz];
int tree[sz*4];
...
int main()
{
    int i,j,t,T,n,c,ans,x,y,q,u,v,temp;
    scin(T);
    RUN_CASE(t,T)
    {
        ms(tree,0);
        ...
        for(i=1; i<=q; i++)
        {
            ...
            u=rgt[arr[x]];
            v=lft[arr[y]];
            if(v-1>=u+1 && (arr[x]!=arr[y]))
            {
                ans=query(1,1,n,u+1,v-1);
            }
            if(arr[x] != arr[y])
            {
                ans=max(ans,abs(u-x)+1);
                ans=max(ans,abs(y-v)+1);
            }
            else
            {
                ans=max(ans,y-x+1);
