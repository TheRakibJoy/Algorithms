...
#define sz 100005
#define qsz 100005
#define k 320  /** Number of block = sqrt(10000) = 320 **/
int ara[sz];
struct query
{
    int l,r,id;
}q[qsz];
bool cmp(query &a,query &b) /** Sort the range for offline calculation**/
{
    int block_a=a.l/k;  /**Count block according to lb of range**/
    int block_b=b.l/k;
    if(block_a == block_b)
        return a.r<b.r;
    return block_a<block_b;
}
int lp=0,rp=-1,sum=0,ans[qsz]; /**lp-left pointer, rp-right pointer**/
void add(int x)
{
    sum += ara[x];
}
