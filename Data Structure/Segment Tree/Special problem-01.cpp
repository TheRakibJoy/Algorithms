...
/** Problem: Finding the maximum value in a range & number of times it appears **/
#define sz 100005
ll ara[sz];
pll tree[4*sz]; ///First-value ; Second-frequency

pll Combine(pll pr1,pll pr2)
{
    if(pr1.first > pr2.first)
        ...
        return make_pair(pr1.first , pr1.second+pr2.second);
}
...
pll Query(ll node,ll b,ll e,ll i,ll j)
{
    ...
    if(b>=i && e<=j)
        return tree[node];
    ll mid = (b+e)>>1;
    pll pr1 = Query(node<<1 , b , mid , i , j);
    pll pr2 = Query(1+(node<<1) , mid+1 , e , i , j);
...
int main()
{
    ...
    for(i=1 ; i<=q ; i++){
        ...
        if(id == 1){
            scln2(l , r);
            pll pr = Query(1 , 1 , n , l , r);
            pf("Maximum value in range = %lld\t It appears = %lld times\n",pr.first,pr.second);
        }
        else{
            scln2(pos , x);
            Update(1 , 1 , n , pos , x);
