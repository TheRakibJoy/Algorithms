...
/** Problem: Searching for the first element greater than a given amount **/
#define sz 100005
...
int Query(int node,int b,int e,int val)
{
    ...
    int mid = (b+e)>>1;
    if(tree[node<<1] >= val)
        Query(node<<1 , b , mid , val);
    else
        Query(1+(node<<1) , mid+1 , e , val);
}

int main()
{
    int i,j,k,n,q,val,pos,cmd;
    scin2(n , q);
