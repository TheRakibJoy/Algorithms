...
/** Range maximum query **/
#define sz 100005
map<int,int>compress;   /** original value to assigned value(Array compression) **/
set<int>st;
set<int>::iterator it;
...
int main()
{
    ...
    RUN_CASE(t,T)
    {
        ...
        for(i=1 ; i<=n ; i++)
            update(1 , 1 , cnt , compress[start[i]] , compress[ed[i]] , 1);
        ans = query(1,1,cnt,1,cnt);
        pf("Case %d: %d\n",t,ans);
        /** Clean **/
        ms(tree,0);
        ms(lazy,0);
