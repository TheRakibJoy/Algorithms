...
struct info
{
    ...
    {
        bal = a; open = b; close = c;
    }
}tree[4*sz];

string s;

info Combine(info lft,info rgt)
{
    info ret;
    int mn = min(lft.open , rgt.close);
    ret.bal = lft.bal + rgt.bal + mn;
    ret.open = lft.open - mn + rgt.open;
    ret.close = lft.close + rgt.close - mn;
    return ret;
}
