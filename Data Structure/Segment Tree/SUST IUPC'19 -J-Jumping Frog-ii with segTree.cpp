...
void init(int node,int b,int e)
{
    ...
    init(left,b,mid);
    init(right,mid+1,e);
    tree[node].mx = min(tree[left].mx,tree[right].mx);
}
...
int main()
{
    int i,j,k,t,T,n,ret,q,id,prev,x,opt;
    scin(T);
    RUN_CASE(t,T)
    {
        ms(ara,0);
        ...
        for(i=1 ; i<=q ; i++)
        {
            ...
            if(opt == 1)
            {
                ...
                prev=ara[id];
                if(prev > 0)
                    update(1,1,n,id+1,id+prev,-1);
                ara[id]=x;
                if(ara[id] > 0)
                    update(1,1,n,id+1,id+ara[id],1);
            }
            else
            {
                scin(x);
                prev=ara[0];
                if(prev > 0)
                    update(1,1,n,0+1,0+prev,-1);
                ara[0]=x;
                if(ara[0] > 0)
                    update(1,1,n,0+1,0+ara[0],1);
                ret = query(1,1,n,1,n);
                if(ret >= 1)
                    pf("yes\n");
                else
