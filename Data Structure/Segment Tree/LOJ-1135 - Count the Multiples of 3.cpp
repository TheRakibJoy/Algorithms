...
void update(int node,int b,int e,int i,int j,int x)
{
    ...
    {
        tree[node].zero=tree[left].two+tree[right].two;
        tree[node].two=tree[left].one+tree[right].one;
        tree[node].one=tree[left].zero+tree[right].zero;
    }
    else if(tree[node].prop%3 == 2)
    {
        tree[node].zero=tree[left].one+tree[right].one;
        tree[node].one=tree[left].two+tree[right].two;
        tree[node].two=tree[left].zero+tree[right].zero;
    }
    else
    {
        tree[node].zero=tree[left].zero+tree[right].zero;
...
int query(int node,int b,int e,int i,int j,int carry=0)
{
    ...
    int right=left+1;
    int mid=(b+e)/2;
    int p1=query(left,b,mid,i,j,(carry+tree[node].prop)%3);
    int p2=query(right,mid+1,e,i,j,(carry+tree[node].prop)%3);
    return p1+p2;
}
void reset()
{
