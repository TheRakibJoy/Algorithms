...
int Query(int node,int b,int e,int i,int j,int val)
{
    ...
    int rgt = 1+lft;
    int mid = (b+e)>>1;
    int p1 = Query(lft , b , mid , i , j , val);
    int p2 = Query(rgt , mid+1 , e , i , j , val);
    return p1+p2;
}

int Bin(int n,int lft,int rgt,int kth)
{
    int lo=-1e9,hi=1e9,mid,ans;
    while(lo <= hi){
        mid = hi-(hi-lo)/2;
        if(Query(1 , 1 , n , lft , rgt , mid) >= kth){
            ans = mid;
            hi = mid-1;
        }
        else
