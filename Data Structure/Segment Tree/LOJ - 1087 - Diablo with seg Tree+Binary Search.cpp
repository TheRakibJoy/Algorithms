/** Segment Tree + Internal Binary Search **/
...
int query(int node,int b,int e,int i,int j,int x)
{
    ...
    if(b != e)
    {
        int id;
        if(tree[left] >= x)
            id = query(left,b,mid,i,j,x);
        else
            id = query(right,mid+1,e,i,j,x-tree[left]);
    }
    else
    {
        if(tree[node] == x)
