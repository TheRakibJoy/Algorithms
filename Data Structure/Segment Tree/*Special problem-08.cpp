...
/** Find the smallest number greater or equal to a specified number in a sorted range,no modification queries ; Merge Sort Tree **/
...
int Query(int node,int b,int e,int i,int j,int val)
{
    if(e<i || b>j)
        return infinity;
    if(b>=i && e<=j){
        int pos = lower_bound(tree[node].begin(),tree[node].end(),val+1)-tree[node].begin();
        if(pos < (int)tree[node].size())
            return tree[node][pos];
        else
            return infinity;
    }
    int mid = (b+e)>>1;
    int p1 = Query(node<<1 , b , mid , i , j , val);
    int p2 = Query(1+(node<<1) , mid+1 , e , i , j , val);
    return min(p1,p2);
}

int main()
{
    int i,j,k,n,q,l,r,val,ans;
    scin2(n , q);
