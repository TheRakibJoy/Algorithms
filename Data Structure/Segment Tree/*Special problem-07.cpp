...
/** Searching for the first element greater than a given value x in a specific range l to r **/
#define sz 100005
int ara[sz],tree[4*sz],lazy[4*sz];
...
int getFirst(int node,int b,int e,int i,int j,int val)
{
    if(e<i || b>j)
        return -1;
    if(b>=i && e<=j){
    ...
    int mid = (b+e)>>1;
    int ret = getFirst(node<<1 , b , mid , i , j , val);
    if(ret != -1)
        return ret;
    else
        return getFirst(1+(node<<1) , mid+1 , e , i , j , val);
}
int main()
{
    int i,j,k,n,q,pos,val,l,r,cmd;
    scin2(n , q);
    ...
    for(i=1 ; i<=q ; i++){
        ...
        if(cmd == 1){
            scin2(l , r);
            scin(val);
            cout<<getFirst(1 , 1 , n , l , r , val)<<endl;
        }
        else{
            scin2(pos , val);
