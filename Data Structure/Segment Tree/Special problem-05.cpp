...
/** Problem: For a given value x you have to find the smallest index i such that the sum of first i elements of the array is grater or equal to x **/
#define sz 100005
...
int Query(int node,int b,int e,int x)
{
    ...
    int mid = (b+e)>>1;
    if(tree[node<<1] >= x)
        return Query(node<<1 , b , mid , x);
    else
        return Query(1+(node<<1) , mid+1 , e , x-tree[node<<1]);
}
