...
#define sz 1000005
int n,ara[sz],nod[sz],mxtree[4*sz];
ll sumtree[4*sz];
void NOD(int n)
{
    for(int i=1 ; i<=n ; i++){
...
void BuildAll(int node,int b,int e)
{
    if(b == e){
        mxtree[node] = ara[b];
        sumtree[node] = ara[b];
    ...
    int mid = (b+e)>>1;
    BuildAll(node<<1 , b , mid);
    BuildAll(1+(node<<1) , mid+1 , e);
    mxtree[node] = max(mxtree[node<<1] , mxtree[1+(node<<1)]);
    sumtree[node] = sumtree[node<<1]+sumtree[1+(node<<1)];
}
int Querymx(int node,int b,int e,int i,int j)
{
    ...
    if(b>=i && e<=j)
        return mxtree[node];
    int mid = (b+e)>>1;
    int p1 = Querymx(node<<1 , b , mid , i , j);
    int p2 = Querymx(1+(node<<1) , mid+1 , e , i , j);
    return max(p1 , p2);
}
ll Querysum(int node,int b,int e,int i,int j)
{
    if(j<b || i>e)
        return 0LL;
    if(b>=i && e<=j)
        return sumtree[node];
    int mid = (b+e)>>1;
    ll p1 = Querysum(node<<1 , b , mid , i , j);
    ll p2 = Querysum(1+(node<<1) , mid+1 , e , i , j);
    return p1+p2;
}
void update(int node,int b,int e,int i,int j)
{
    if(j<b || i>e)
        return;
    if(b>=i && e<=j && b==e){
        mxtree[node] = nod[mxtree[node]];
        sumtree[node] = nod[sumtree[node]];
        return;
    }
    int mid = (b+e)>>1;
    int p1 = Querymx(1 , 1 , n , b , mid);
    int p2 = Querymx(1 , 1 , n , mid+1 , e);
    if(p1 > 2)
