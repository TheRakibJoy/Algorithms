...
map<int,int>ara;    /**Save the frequency of array element value**/
map<int,int>m;      /**Save the node no. of each element**/
int tree[300005];
int past[100005],nxt[100005];
...
{
//    Output;
    int i,j,n,q,ans,x,y,cnt=1,k=0,temp,temp1,left,right;
    ans=0;
    while(1)
    {
        ara.clear();
...
//    cout<<endl;
        ...
        for(j=1; j<=q; j++)
        {
            ...
            {
                if(m[temp] <= m[temp1])
                    ans=query(1,1,k,m[temp],m[temp1]);
            ans=max(ans,temp-x);
            ans=max(ans,y-temp1);
            }
            else
