...
int Combine(int mod,int level,int lft,int rgt)
{
    if(mod == 0){
        if(level%2 == 0)
...
void Build(int mod,int level,int node,int b,int e)
{
    if(b == e){
        tree[node] = ara[b];
        return;
    ...
    int lft = node<<1;
    int rgt = lft+1;
    Build(mod , 1+level , lft , b , mid);
    Build(mod , 1+level , rgt , mid+1 , e);
    tree[node] = Combine(mod , level , lft , rgt);
}

void Update(int mod,int level,int node,int b,int e,int i,int j,int val)
{
    if(e<i || b>j)
        return;
    if(b>=i && e<=j){
    ...
    int lft = node<<1;
    int rgt = lft+1;
    Update(mod , 1+level , lft , b , mid , i , j , val);
    Update(mod , 1+level , rgt , mid+1 , e , i , j , val);
    tree[node] = Combine(mod , level , lft , rgt);
}

void Solve(int t)
{
    int i,j,k,n,q,id,val,ans,tot;
    scin2(n , q);
    tot = 1<<n;
    for(i=1 ; i<=tot ; i++)
        scin(ara[i]);
    ...
    for(i=1 ; i<=q ; i++){
        scin2(id , val);
        Update(n%2 , 1 , 1 , 1 , tot , id , id , val);
        ans = tree[1];
        pf("%d\n",ans);
    }
}
