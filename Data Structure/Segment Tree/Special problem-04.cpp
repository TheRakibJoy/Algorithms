...
/** Problem: Count the number of zeros in a given range & find the index of kth zero **/
#define sz 100005
...
int CountZero(int node,int b,int e,int i,int j)
{
    if(e<i || b>j)
        return 0;
    if(b>=i && e<=j)
        return tree[node];
    int mid = (b+e)>>1;
    int p1 = CountZero(node<<1 , b , mid , i , j);
    int p2 = CountZero(1+(node<<1) , mid+1 , e , i , j);
    return p1+p2;
}
int kthZero(int node,int b,int e,int k)
{
    if(k > tree[node])
        return -1;
    if(b == e)
    ...
    int mid = (b+e)>>1;
    if(tree[node<<1] >= k)
        return kthZero(node<<1 , b , mid , k);
    else
        return kthZero(1+(node<<1) , mid+1 , e , k-tree[node<<1]);
}
...
int main()
{
    int i,j,k,n,q,pos,val,cmd,ans,kth,l,r;
    scin2(n , q);
    ...
    for(i=1 ; i<=q ; i++){
        ...
        if(cmd == 1){
            scin2(l , r);
            ans = CountZero(1 , 1 , n , l , r);
            pf("There're %d zeros in the given range.\n",ans);
        }
        else if(cmd == 2){
            scin(k);
            kth = kthZero(1 , 1 , n , k);
            pf("Index of %dth zero = %d\n",k,kth);
        }
        else{
            scin2(pos , val);
