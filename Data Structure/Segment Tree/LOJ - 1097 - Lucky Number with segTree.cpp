...
int kthLucky(int node,int b,int e,int k)
{
    if(k > tree[node])
        return -1;
    if(b == e)
    ...
    int mid = (b+e)>>1;
    if(tree[node<<1] >= k)
        return kthLucky(node<<1 , b , mid , k);
    else
        return kthLucky(1+(node<<1) , mid+1 , e , k-tree[node<<1]);
}
...
void Update(int node,int b,int e,int pos)
{
    ...
    int mid = (b+e)>>1;
    Update(node<<1 , b , mid , pos);
    Update(1+(node<<1) , mid+1 , e , pos);
...
void PreCal()
{
    ...
    for(int i=2 ; i<=limit ; i++){
        int bad = 0;
        int ithnum = kthLucky(1 , 1 , limit , i);
        if(ithnum == -1)
            break;
        for(int j=ithnum ; j<=limit ; j+=ithnum){
            int id = kthLucky(1 , 1 , limit , j-bad);
            if(id == -1)
                break;
            Update(1 , 1 , limit , id);
            bad++;
