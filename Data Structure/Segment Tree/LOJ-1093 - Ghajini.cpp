/** Range max query+Range min query+check query for each node **/
#include<bits/stdc++.h>
...
int mxquery(int node,int b,int e,int i,int j)
{
    if(i>e || j<b)
        return 0;
    if(b>=i && e<=j)
    ...
    int right=left+1;
    int mid=(b+e)/2;
    int p1=mxquery(left,b,mid,i,j);
    int p2=mxquery(right,mid+1,e,i,j);
    return max(p1,p2);
}
int mnquery(int node,int b,int e,int i,int j)
{
    if(i>e || j<b)
        return 999999999;
    if(b>=i && e<=j)
    ...
    int right=left+1;
    int mid=(b+e)/2;
    int p1=mnquery(left,b,mid,i,j);
    int p2=mnquery(right,mid+1,e,i,j);
    return min(p1,p2);
}

int main()
{
    int i,j,t,T,ans,u,v,d,n,p1,p2,cnt;
    scin(T);
    RUN_CASE(t,T)
    {
        ans=0;
        ...
        for(i=1;i<=n;i++)
        {
            p1=mnquery(1,1,n,i,min(n,i+d-1));
            p2=mxquery(1,1,n,i,min(n,i+d-1));
            cnt=abs(p2-p1);
            ans=max(ans,cnt);
        }
        pf("Case %d: %d\n",t,ans);
