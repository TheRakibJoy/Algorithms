...
int query(int node,int b,int e,int x)
{
    ...
    if(tree[left] >= x)
        ...
        return query(right,mid+1,e,x-tree[left]);
}
...
int main()
{
    int i,j,k,t,T,x,y,ans,q,id,lft,rgt,temp;
    scin(T);
    RUN_CASE(t,T)
    {
        ms(ara,0);
        ...
        for(i=1; i<=q; i++)
        {
            ...
            rgt=ara[id].r;
            ara[id].val=y;
            if(id!=lft && id!=rgt && ara[id].val==ara[lft].val && ara[id].val==ara[rgt].val)
            {
                ara[lft].val=-1;
                ara[id].val=-1;
                update(1,1,n,id);
            ...
            else if(id!=lft && ara[id].val == ara[lft].val)
            {
                ara[id].val=-1;
                update(1,1,n,id);
                ara[rgt].l=lft;
            ...
            else if(id!=rgt && ara[id].val == ara[rgt].val)
            {
                ara[id].val=-1;
                update(1,1,n,id);
                ara[rgt].l=lft;
