...
/**
    a[i] ^ a[i+1] ^...^ a[j] = k
or, cXOR[j] ^ cXOR[i-1] = k
or, cXOR[i-1] = cXOR[j] ^ k ; For each j find occurrence of cXOR[i-1]
**/

#define sz 100099
int n,q,k,blocksz,ara[sz],cxor[sz],fre[3000009];
ll tot = 0 , ans[sz];
...
void Solve(int t)
{
    ...
    for(i=1 ; i<=n ; i++){
        scin(ara[i]);
        cxor[i] = cxor[i-1] ^ ara[i];
    }

    for(i=1 ; i<=q ; i++){
        scin2(query[i].l , query[i].r);
        query[i].l--;   ///As CXOR[i]^CXOR[i-1] = XOR[i], that means minimum size of the range is 2
        query[i].id = i;
    }
