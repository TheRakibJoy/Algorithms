...
#define sz 1000009
int blocksz , mx=0 , sum =0 , lft[sz] , rgt[sz] , startat[sz] , endat[sz];
set<int>st;
...
void Solve(int t)
{
    ...
    blocksz = sqrt(cnt);
    for(i=0 ; i<n ; i++){
        query[i].l = compress[lft[i]];
        query[i].r = compress[rgt[i]];
        query[i].id = i;
    ...
    for(i=0 ; i<n ; i++){
        ...
        while(l < query[i].l)   Removel(l++);
        while(r > query[i].r)   Remover(r--);
