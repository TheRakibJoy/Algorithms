...
/** You're given a graph contains n node & m edges. You've to operate two different queries:
    ...
    q2. Answer the total node cost between two anchestor & decendands u & v
    You've to perform 10^5 queries
**/

vi graph[30005];
bool vis[30005];
int st[30005],ed[30005],cost[30005],curr_time;  /** discovered time & finish time of each node & node cost **/
void DFS(int start)
{
    vis[start]=1;
    st[start] = ++curr_time;
...
int main()
{
    int n,m,u,v,q,i,j,k,ans,start,finish,id;
    scin2(n,m);
    ...
    for(i=1 ; i<=q ; i++)
    {
        ...
        {
            update(1,1,2*n,st[u],v);    /** update the cost of node u by v **/
