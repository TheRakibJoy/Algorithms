...
/**
Problem:    0-1 BFS Template
You'll given a graph with n vertex and m edges. What is the minimum number of edges
you need to reverse in order to have at least one path from vertex 1 to n.
**/
#define sz 100005
vi graph[sz],cost[sz];
int dis[sz];
...
int main()
{
    int i,j,k,n,e,u,v,ans;
    scin2(n , e);
    for(i=1 ; i<=e ; i++){
        scin2(u , v);
        graph[u].pb(v);
        graph[v].pb(u);
        cost[u].pb(0);  /// There is an edge from u to v
        cost[v].pb(1);  /// There will be an edge from v to u if I invert the edge u to v
    }
    BFS(1);
    ans = dis[n];
    if(ans == infinity)
