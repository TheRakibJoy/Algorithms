...
int MST(int n)
{
    ...
    for(int i=0 ; i<e.size() ; i++)
    {
        ...
        if(u != v)
        {
            ...
            cnt+=1;
            tot += e[i].cost;
            temp.pb(e[i]);      /** I don't need those edges that already rejected ; So, I need these edges used here **/
            if(cnt == n-1)
                break;
        }
    }
    e.clear();
    e=temp;         /** Here are the great edge optimization  **/
    if(cnt == n-1)
        return tot; /** All the nodes are connected **/
    else
        return -1;  /** There are twice or more subgraph **/
}
int main()
{
//    Output;
    int i,j,k,n,w,t,T,ans,cnt=0;
    scin(T);
    RUN_CASE(t,T)
    {
        scin2(n,w);
        ...
        {
            edge take;
            sc("%d %d %d",&take.u,&take.v,&take.cost);
            e.pb(take);
            ans = MST(n);
            pf("%d\n",ans);
        }
