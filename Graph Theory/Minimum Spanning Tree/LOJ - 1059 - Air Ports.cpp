...
int vis[100005],par[100005],a,xtra;
struct edge
{
    int u,v,w;
...
{
//    Output;
    int i,j,k,n,m,x,y,c,subgraph,t,T;
    ll tot,ans,temp;
    edge take;
    scin(T);
    RUN_CASE(t,T)
    {
        ...
        for(i=1 ; i<=subgraph ; i++)
        {
            if(ed[i].size() > 0)
            {
                temp = MST(node[i],ed[i]);
                tot += temp;
            }
        }
        ans = tot+(subgraph*a);
