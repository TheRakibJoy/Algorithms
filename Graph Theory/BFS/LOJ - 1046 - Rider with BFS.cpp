...
char grid[12][12];
int totk,row,col,dis[12][12],vis[12][12];
...
int BFS(int sx,int sy,int no)
{
    ...
    while(!q.empty()){
        ...
        q.pop();
        for(int i=0 ; i<8 ; i++){
            int nx = pr.first+fx[i];
            int ny = pr.second+fy[i];
            if(nx>=0 && nx<row && ny>=0 && ny<col && vis[nx][ny]!=no){
                vis[nx][ny] = no;
                dis[nx][ny] = 1+dis[pr.first][pr.second];
                q.push(make_pair(nx , ny));
                if(grid[nx][ny] != '.'){
                    int mov = (grid[nx][ny]-'0');
                    cnt++;
                    ret += ((dis[nx][ny]+mov-1)/mov);
                }
            }
        }
    }
