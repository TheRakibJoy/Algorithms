...
#define mod 26
int dis[28][28][28];
bool vis[28][28][28],isblock[28][28][28];
...
void BFS(int sx,int sy,int sz)
{
    ...
    while(!q.empty()){
        ...
        q.pop();
        for(int i=0 ; i<6 ; i++){
            int nx = (u.x+fx[i]+mod)%mod;
            int ny = (u.y+fy[i]+mod)%mod;
            int nz = (u.z+fz[i]+mod)%mod;
            if(!isblock[nx][ny][nz] && !vis[nx][ny][nz]){
                vis[nx][ny][nz] = 1;
                dis[nx][ny][nz] = 1+dis[u.x][u.y][u.z];
...
{
    Clean();
    int xx,i,j,k,n,sz1,sz2,sz3,x1,x2,x3,ans;
    char start[5],finish[5],s1[30],s2[30],s3[30];
//    getchar();
    sc("%s %s",start,finish);
    scin(n);
    for(xx=1 ; xx<=n ; xx++){
