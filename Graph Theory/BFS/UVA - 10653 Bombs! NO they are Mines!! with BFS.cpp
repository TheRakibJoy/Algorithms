...
char grid[1005][1005];
int row,col,dis[1005][1005],vis[1005][1005];
void Clean()
{
...
int main()
{
    int i,j,k,num_row,row_loc,num_bomb,col_loc,ans,sx,sy,dx,dy;
    while(1)
    {
        Clean();
        scin2(row,col);
        ...
        for(i=1 ; i<=num_row ; i++)
        {
            scin2(row_loc,num_bomb);
            for(j=1 ; j<=num_bomb ; j++)
            {
                scin(col_loc);
                grid[row_loc][col_loc]='#';
            }
        }
        scin2(sx,sy);
        scin2(dx,dy);
