...
#define sz 100005
set<int>nonvis;
vector<int>notconn[sz];    ///1-cost edges

void DFS(int u)
{
    for(int i=0 ; i<(int)notconn[u].size()-1 ; i++){
        int lft = notconn[u][i] + 1;
        int rgt = notconn[u][i+1] - 1;
        if(lft > rgt)
            continue;
        while(1){
...
void Solve(int t)
{
    ...
    for(i=1 ; i<=n ; i++){
        notconn[i].pb(0);
        notconn[i].pb(n+1);
        sort(notconn[i].begin() , notconn[i].end());
        nonvis.insert(i);
    }
    for(i=1 ; i<=n ; i++){
