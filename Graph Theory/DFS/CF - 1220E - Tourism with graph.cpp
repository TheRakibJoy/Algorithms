...
int vis[200005],wt[200005],par[200005];
vi graph[200005];

void DFS(int u , int parent)    ///cycle related node gulo sob mark koro
{
    vis[u] = 1;
    par[u] = parent;
    for(int i=0 ; i<(int)graph[u].size() ; i++){
        int v = graph[u][i];
        if(vis[v]==1 && v!=parent && vis[u]!=2)
            vis[v] = 2;
        if(vis[v] == 0)
            DFS(v , u);
        if(vis[v] == 2 && v!=parent)    ///Starting theke cycle er dike jete sob node k mark korteci
            vis[u] = 2;
    }
}
...
void DFS2(int u)    ///Starting maximum cost er leaf er dike path konta ber koro
{
    vis[u] = 1;
    ll mx = 0;
    for(int i=0 ; i<(int)graph[u].size() ; i++){
