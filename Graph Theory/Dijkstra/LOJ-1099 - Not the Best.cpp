...
void dijkstra(int s)
{
    ...
    while(!pq.empty())
    {
        ...
        pq.pop();
        int u=top.u;
        if(top.w>d[1][u]) continue;
        for(int i=0;i<graph[u].size();i++)
        {
            int v=graph[u][i];
            if(top.w+cost[u][i] < d[0][v])
            {
                d[1][v]=d[0][v];
                d[0][v]=top.w + cost[u][i];
                pq.push(node(v,d[0][v]));
            }
            else if(top.w+cost[u][i] < d[1][v] && top.w+cost[u][i] > d[0][v])
            {
                d[1][v]=top.w+cost[u][i];
