...
#define sz 100005
vl graph[sz],rgraph[sz],cost[2][sz];
struct node
{
    ll u,w;
    node(ll a,ll b){
...
ll d[2][sz];    ///d[0]-graph   ;   d[1]-rgraph
ll par[2][sz];
void Clean(ll n)
{
    for(ll i=0 ; i<=n ; i++){
...
void dijkstra(ll start)
{
    ...
    while(!pq.empty()){
        ...
        for(ll i=0 ; i<(ll)graph[u].size() ; i++){
            ll v = graph[u][i];
            if(top.w+cost[0][u][i] < d[0][v]){
                d[0][v] = top.w+cost[0][u][i];
                pq.push(node(v , d[0][v]));
                par[0][v] = u;
            }
        }
    }
}
void rdijkstra(ll start)
{
    priority_queue<node>pq;
    pq.push(node(start , 0));
    d[1][start] = 0;
    while(!pq.empty()){
        ...
        if(top.w > d[1][u])
            continue;
        for(ll i=0 ; i<(ll)rgraph[u].size() ; i++){
            ll v = rgraph[u][i];
            if(top.w+cost[1][u][i] < d[1][v]){
                d[1][v] = top.w+cost[1][u][i];
                pq.push(node(v , d[1][v]));
                par[1][v] = u;
            }
        }
    }
...
int main()
{
    ll i,j,k,n,t,T,m,x,q,start,des,ans,w,u,v;
    scln(T);
    Clean(100002);
    RUN_CASE(t,T)
    {
        ...
        {
            scln2(start,des);
            ans = d[1][start]+d[0][des];
            if(ans >= longlimit)
                pf("Be seeing ya, John\n");
            else
                pf("%lld\n",ans);
        }
