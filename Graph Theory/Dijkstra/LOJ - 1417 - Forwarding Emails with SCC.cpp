...
#define sz 50005
int compno[sz],compsz[sz],vis[sz];
vi graph[sz],rgraph[sz],comp[sz];
stack<int>stk;
queue<int>qu;
...
void DFS2(int start,int x)
{
    ...
    compno[start] = x;
    comp[x].pb(start);
    for(int i=0 ; i<(int)rgraph[start].size() ; i++){
...
void Solve(int t)
{
    int i,j,k,n,u,v,ans=0,x=0,desiredcomp=0,mxsz=0;
    scin(n);
    for(i=1 ; i<=n ; i++){
        scin2(u,v);
        graph[u].pb(v);
