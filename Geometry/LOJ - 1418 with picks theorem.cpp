...
ll twotimes_Area(ll n)
{
    ll i,j,k,part1=0,part2=0;
    for(i=0 ; i<n ; i++){
        part1 += (Point[i].first * Point[i+1].second);
        part2 += (Point[i].second * Point[i+1].first);
    }
    return fabs(part1 - part2);
}

ll PointsLies(pll p1,pll p2)
{
    if(p1.second == p2.second)
        return (fabs(p1.first-p2.first) - 1LL);
    else if(p1.first == p2.first)
        return (fabs(p1.second-p2.second) - 1LL);
    else{
        ll x = fabs(p1.first - p2.first);
        ll y = fabs(p1.second - p2.second);
        ll gc = gcd(x , y);
        return gc-1LL;
    }
}
...
void Solve(int t)
{
    Point.clear();
    ll i,j,k,x,y,n;
    ll Area2x,b = 0,a,inside;
    scln(n);
    for(i=1 ; i<=n ; i++){
        scln2(x , y);
        Point.pb(make_pair(x , y));
    }
    Point.pb(make_pair(Point[0].first , Point[0].second));
    ///Calculate total point in boundary
    b += n;
    for(i=0 ; i<n ; i++){
        b += PointsLies(Point[i] , Point[i+1]);
    }

    Area2x = twotimes_Area(n);
    a = Area2x;
    inside = (a-b+2)/2;
    pf("Case %d: %lld\n",t,inside);
}
